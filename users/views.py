...
def view_friends(request, username, template_name="friend/user_list.html"):
...
def friendship_add_friend(
    request, to_username, template_name="friend/add.html"
):
...
def friendship_request_list(
    request, template_name="friend/requests_list.html"
):
...
def friendship_requests_detail(
    request, friendship_request_id, template_name="friend/request.html"
):
...
def followers(request, username, template_name="follow/followers_list.html"):
...
def following(request, username, template_name="follow/following_list.html"):
...
@login_required
def follower_add(
    request, followee_username, template_name="follow/add.html"
):
...
@login_required
def follower_remove(
    request, followee_username, template_name="follow/remove.html"
):
...
def all_users(request, template_name="user_actions.html"):
    users = user_model.objects.all()
...
def blocking(request, username, template_name="block/blockers_list.html"):
    """ List this user's followers """
    user = get_object_or_404(user_model, username=username)
...
def blockers(request, username, template_name="block/blocking_list.html"):
    """ List who this user follows """
    user = get_object_or_404(user_model, username=username)
...
@login_required
def block_add(request, blocked_username, template_name="block/add.html"):
    """ Create a following relationship """
...
@login_required
def block_remove(
    request, blocked_username, template_name="block/remove.html"
):
    """ Remove a following relationship """
    if request.method == "POST":
        blocked = user_model.objects.get(username=blocked_username)
